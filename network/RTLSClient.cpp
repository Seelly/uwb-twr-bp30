...
#if (DEBUG_FILE==1)
    QString filenameDbg(now.toString("yyyyMMdd_hhmmss")+"RTLS_log_dbg.txt");
...
// post
void RTLSClient::Postdata(QString g){
...
//       string s = g.toStdString();
        ...
        qDebug() << "\ng:" << g;
       // 将输入数据按空格分割成字符串列表
       QStringList tokens = g.split(" ");

       qDebug() << "tokens:" << tokens;
       // 创建一个 QJsonObject 对象，并添加键值对
       QJsonObject jsonObj;
       for (const auto& token : tokens) {
           QStringList keyValuePair = token.split(":");
           if (keyValuePair.size() >= 2) {
               jsonObj.insert(keyValuePair[0], keyValuePair[1]);
           }
       }

       // 将 QJsonObject 对象转换成 QJsonDocument 类型
       QJsonDocument jsonDoc(jsonObj);
       QByteArray postData = jsonDoc.toJson();
       // 创建一个 QNetworkRequest 对象，并指定请求 URL
       QNetworkRequest request;
       request.setUrl(QUrl("http://localhost:8000/api/ws"));
       // 设置请求头
       ...
       QObject::connect(reply, &QNetworkReply::finished, [=]() {
           ...
           reply->deleteLater();
           manager->deleteLater();  // 在请求完成后释放 QNetworkAccessManager 对象
       });
//        delete manager;
       return;
}
...
void RTLSClient::newData()
{
    ...
    uint16 ii,raw[7],i;
    uint8 tmp,TAG_ID,change_line=0;
    static uint16 store_pos=0,read_pos=0,data_pos,usart_state=0;
    uint16 range[3],seq,lnum;
...
//_serial->write(data);
    for(i=0; i<length; i++)
    {
        cache[store_pos++]=data[i];
        if(store_pos>cache_len)
        {
            store_pos = 0;
        }
    }
    while(store_pos!=read_pos)
    {
        tmp = cache[read_pos++];
        if(read_pos>cache_len)
        {
            read_pos=0;
        }
        switch(usart_state)
...
//if(store_pos+length<512)
//{
//    memcpy(&cache[store_pos],data,length);
//    store_pos+=length;
//}else{
//    memcpy(cache2,data,length);
//    memcpy(&cache[store_pos],cache2,512-store_pos);
//    memcpy(&cache[0],&cache2[512-store_pos],length-(512-store_pos));
//    store_pos=length-(512-store_pos);
//    change_line=1;
//}
//if(change_line)
//{
...
//while(store_pos-read_pos>frame_length)
//{
//        tmp = cache[read_pos++];
//        if(tmp==214)
//        {
...
//                 tmp = cache[read_pos++];
//                TAG_ID = cache[read_pos++];
//                if(tmp==1)
//                {
//                    lnum=cache[read_pos++]|(cache[read_pos++]<<8);
//                    read_pos+=2;
//                    seq = lnum&0xff;
//                    range[0] = (uint8)cache[read_pos++]|((uint8)cache[read_pos++]<<8);
//                    range[1] = (uint8)cache[read_pos++]|((uint8)cache[read_pos++]<<8);
//                    range[2] = (uint8)cache[read_pos++]|((uint8)cache[read_pos++]<<8);
//                    range[0] = range[0]*10;
//                    range[1] = range[1]*10;
//                    range[2] = range[2]*10;
...
//                    ra[1] = range[1]*10;
//                    ra[2] = range[2]*10;
//                    processTagRangeReport(0, TAG_ID, range[0], lnum, seq);
//                    processTagRangeReport(1, TAG_ID, range[1]*10, lnum, seq);
//                    processTagRangeReport(2, TAG_ID, range[2]*10, lnum, seq);
//                    trilaterateTag(TAG_ID, seq);
//                }
...
//                   if(tmp==1)
//                   {
//                       lnum=data[ii++]|(data[ii++]<<8);
//                       seq = lnum&0xff;
//                       ii=ii+2;
//                       range[0] = (uint8)data[ii++]|((uint8)data[ii++]<<8);
//                       range[1] = (uint8)data[ii++]|((uint8)data[ii++]<<8);
//                       range[2] = (uint8)data[ii++]|((uint8)data[ii++]<<8);
//                       if(_useAutoPos) //if Anchor auto positioning is enabled then process Anchor-Anchor TWR data
//                       {
//                           processAnchRangeReport(0, 1, range[0]*10, lnum, seq);
//                           processAnchRangeReport(0, 2, range[1]*10, lnum, seq);
//                           processAnchRangeReport(1, 2, range[2]*10, lnum, seq);
